...
export default function AjoutCours(){
  ...
  try {
    const response = await fetch('http://localhost:6969/video', {
      method: 'POST',
    ...
    return (
           <div id="conteneurForm" className=" font-sans  md:font-serif  me-0.5 mt-3  bg-slate-100 w-3/6 ml-96">
            ...
            <input onChange={(e)=>setCible(e.target.value)}  name='niveau'   className="ml-2  w-96 border-2 border-4 border-indigo-200 border-x-indigo-500"  type="text"  /> 
            </div>
            <div className="mt-7">
            ...
            <textarea onChange={(e)=>setText(e.target.value)}  className="rounded-md bg-gray-200  w-full h-96 outline-blue-800 border-2 border-4 border-indigo-200 border-x-indigo-500" name="text" type="text"  />
            </div>
            <button className="bg-indigo-500 text-zinc-50 text-center rounded-md w-20 h-10" onClick={()=>{uploadFile()}}>Publier</button>
           </div> 
    );
} ;